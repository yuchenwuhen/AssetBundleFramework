        ...
        public Result Recognize(GestureLibrary gestureLibrary, bool useProtractor = false) {
            ...
            if (this.Points.Count <= 2) {
                ...
                for (int i = 0; i < library.Count; i++) {
                    ...
                    var result = new Result(library[i].Name, useProtractor ? 1f / distance : 1f - distance / this.HALF_DIAGONAL);
                    Debug.Log("Gesture is recognized as <color=#ff0000>'" + result.Name + "'</color> with a score of " + result.Score);
                }
