...
//				}
    ...
    private Dictionary<int, List<Rect>> m_allRectDict = new Dictionary<int, List<Rect>>();

    private int orderLayer = 0;

    private IEnumerator CreateGUI(Sprite[] atlas)
    {
        Debug.Log("创建GUI:" + psd.Layers.Count);
        int index = 0;
        for (var i = 0; i < psd.Layers.Count; i++)
        {
            index++;
            Debug.Log("设置物体数据完成度" + index / (float)psd.Layers.Count);
            yield return null;
                ...
                .SingleOrDefault(x => x is LayerSectionInfo) == null) && (psd.Layers[i].Visible))
            {
                ...
                //else
                {
                    ...
                    {
                        instant.AddComponent<SpriteRenderer>();
                        instant.GetComponent<SpriteRenderer>().sprite =
                            atlas[Array.FindIndex(atlas, x => x.name == RealLayerName)];
                        //instant.GetComponent<SpriteRenderer>().SetNativeSize();
                    }
                    ...
                    var layerRect = psd.Layers[i].Rect;

                    //计算OrderLayer
                    if (!m_allRectDict.ContainsKey(orderLayer))
                    {
                        //如果orderLayer 不存在， layerRect存在缓存中
                        List<Rect> rects = new List<Rect>();
                        m_allRectDict.Add(orderLayer,rects);
                        m_allRectDict[orderLayer].Add(layerRect);
                    }
                    else
                    {
                        bool isOverlap = false;
                        foreach (var item in m_allRectDict[orderLayer])
                        {
                            if (IsOverlap(layerRect,item))
                            {
                                isOverlap = true;
                                break;
                            }
                    ...
                    if (instant.GetComponent<SpriteRenderer>() != null)
                    instant.GetComponent<SpriteRenderer>().sortingOrder = orderLayer;

                    var screenPos = new Vector3(layerRect.x + layerRect.width / 2f - screenRect.width/2f,
                        layerRect.y + layerRect.height / 2f - screenRect.height / 2f,
                        0);
                    instant.transform.position = new Vector3(screenPos.x / 100f, (screenRect.height - screenPos.y) / 100f, 0);

                    //                    instant.transform.position = new Vector3(psd.Layers[i].Rect.xMin,
                    //                        CanvasObj.GetComponent<RectTransform>().rect.height/2 - psd.Layers[i].Rect.yMax, 0);

                    instant.transform.SetParent(temp);
...
//        }
    ...
    private GameObject CreatePanel(string[] path)
    {
        ...
        temp1.transform.position = new Vector3(0, 0, 0);
        temp1.name = pathtemp[pathtemp.Count - 1];

        temp1.transform.SetParent(PathObj[pathtemp.Count - 2].transform);
        PathObj.Add(temp1);
    ...
    {
        LayerList = new List<string>();
        atlas = new List<Sprite>();

        var path = AssetDatabase.GetAssetPath(image).Split('.')[0];
        Directory.CreateDirectory(path);

        //Thread createObj = new Thread(() => { CreateGameObject(); });
        //createObj.Start();

        EditorCoroutineUtility.StartCoroutineOwnerless(CreateGameObject());
    ...
    IEnumerator CreateGameObject()
    {
        ...
        foreach (var layer in psd.Layers)
        {
            ...
            Debug.Log("创建物体完成度" + index / (float)psd.Layers.Count);
            yield return null;
        ...
        //Thread createGUI = new Thread(()=> { CreateGUI(atlas.ToArray());  } );
        //createGUI.Start();

        EditorCoroutineUtility.StartCoroutineOwnerless(CreateGUI(atlas.ToArray()));
    }
